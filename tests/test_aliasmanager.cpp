...
static void testFormatAlias() { AliasManager manager(ShellDetector::Shell::BASH); Alias a{"ll", "ls -la"}; std::string f = manager.formatAlias(a); assert(f.find("alias ll") != std::string::npos); }
static void testParseAliasLine() { Alias a = AliasManager::parseAliasLine("alias ll='ls -la'"); assert(a.name == "ll"); assert(a.command == "ls -la"); }
